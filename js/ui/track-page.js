...
function renderOrder(root, order) {
    ...
    const placeText =
        order.orderType === "dine-in" && order.tableNumber
            ? t("tracking.table", { number: escapeHtml(order.tableNumber) })
    ...
    root.innerHTML = `
        <div style="border:2px solid var(--color-accent); background:var(--color-surface); padding:24px; font-family:'Courier New', monospace;">
            <div style="font-size:12px; color:var(--color-text-muted); text-transform:uppercase; letter-spacing:1px; margin-bottom:4px;">${t("tracking.trackingOrderLabel")}</div>
            <h1 style="margin:0 0 16px; font-size:22px;">#${escapeHtml(order.orderNumber)}</h1>
            <div style="display:inline-block; padding:6px 14px; border:1px solid ${color}; color:${color}; font-weight:bold; letter-spacing:1px; margin-bottom:16px;">${escapeHtml(t(`myOrders.status.${order.status}`))}</div>
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; border-top:1px solid var(--color-accent); padding-top:14px;">
                <span style="font-size:12px; color:var(--color-text-muted);">${order.isPaid ? t("tracking.paid") : t("tracking.paymentDue")}</span>
                <span style="font-size:16px; font-weight:bold; color:var(--color-accent);">${currencySymbol()}${order.total.toFixed(2)}</span>
            </div>
            <p style="font-size:11px; color:var(--color-text-muted); margin:20px 0 0; text-align:center;">${t("tracking.autoUpdateNote")}</p>
        </div>
    `;
}
...
function renderError(root, message) {
    root.innerHTML = `
        <div style="border:2px solid var(--color-danger); background:var(--color-surface); padding:24px; text-align:center; font-family:'Courier New', monospace;">
            <h1 style="margin:0 0 10px; font-size:18px; color:var(--color-danger);">${t("tracking.linkInvalidTitle")}</h1>
            <p style="color:var(--color-text-muted); font-size:13px;">${escapeHtml(message)}</p>
            <button id="track-error-home-btn" class="btn-primary" style="margin-top:14px; background: var(--color-accent); color: var(--color-accent-contrast); border: none; padding: 12px 20px; font-weight: bold; cursor: pointer; font-family: inherit;">${t("tracking.goToMenu")}</button>
        </div>
    `;
...
async function fetchAndRender(root, token) {
    try {
        ...
        if (!res.ok) {
            ...
            stopTrackPolling();
            renderError(root, data.error || t("tracking.linkInvalidFallback"));
            return;
        }
...
export function renderTrackPage(token) {
    ...
    stopTrackPolling();
    if (!token) {
        renderError(root, t("tracking.noCodeGiven"));
        return;
    }
    root.innerHTML = `<p style="color:var(--color-text-muted); font-family:'Courier New', monospace;">${t("common.loading")}</p>`;
